...
public abstract class VisionProcessorBase<T> implements VisionImageProcessor {
  ...
  public synchronized void processImageProxy(ImageProxy image) {
    long frameStartMs = SystemClock.elapsedRealtime();
    if (isShutdown) {
      image.close();
      return;
...
//            graphicOverlay,
  ...
  private Task<T> requestDetectInImage(
    ...
    return setUpListener(
        detectInImage(image), originalCameraImage, image, frameStartMs);
  }
...
//                  .show();
  ...
  protected abstract void onSuccess(@NonNull T results, Bitmap bitmap);
